...
_displayManager(displayManager), _currentState(DISPLAY_INVERTER_POWER), _lastUpdateTime(0), _isActive(false), _needsDisplayUpdate(true) {
...
void EnergyDisplay::updateDisplay(const Inverter& inverter) {
    if (!_isActive || !_needsDisplayUpdate) return;
    ...
    if (currentTime - _lastUpdateTime >= _intervals[_currentState]) {
        ...
        // Proceed with state-specific display logic
        ...
        _needsDisplayUpdate = false; // Reset the flag after updating
...
void EnergyDisplay::start() {
    _isActive = true;
    _needsDisplayUpdate = true; // Ensure immediate update on start
